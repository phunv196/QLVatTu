...
import WarehouseApi from "@/api/warehouse-api"; // eslint-disable-line import/no-cycle
import WarehouseDetails from "@/views/warehouse/WarehouseDetails.vue";
import Import from "@/views/warehouse/Import.vue";
...
export default defineComponent({
  setup(): unknown {
    ...
    const getData = async (
      ...
      try {
        const resp = await WarehouseApi.getWarehouses(
          page,
          requestedPageSize,
          warehouseId,
          code,
    ...
    const confirmDialog = (rec: Record<string, unknown>) => {
      confirm.require({
        ...
        accept: async () => {
          try {
            const resp = await WarehouseApi.deleteWarehouse(
              rec.warehouseId as string
            );
