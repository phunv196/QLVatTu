  ...
  <div class="m-font-regular">
    ...
    <h4>
      ...
      <span style="color: var(--primary-color)">
        {{ recData.positionId ? recData.positionId : "NEW" }}
      </span>
    </h4>
...
export default defineComponent({
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      let msg: any[];
      msg = [];
      if (!rawPositionObj.code) {
        msg.push("mã chức vụ");
      }
      if (!rawPositionObj.name) {
        msg.push("tên chức vụ");
      }
      if (msg.length > 0) {
        userMessage.value =
        ...
        const check = await PositionApi.getPositionByCode(rawPositionObj);
        if (check.data) {
          userMessage.value = "Mã chức vụ bị trùng. Vui lòng nhập lại!";
          showMessage.value = true;
          setTimeout(() => {
            return (showMessage.value = false);
          }, 2000);
          ...
          if (resp.data.msgType === "SUCCESS") {
            toast.add({
              severity: "success",
              summary: rawPositionObj.positionId
                ? "Sửa thành công!"
                : "Thêm mới thành công!",
