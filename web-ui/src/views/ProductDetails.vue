  ...
  <div class="m-font-regular">
    ...
    <h4>
      ...
      <span style="color: var(--primary-color)">
        {{ recData.id ? recData.id : "NEW" }}
      </span>
    </h4>
    <span class="m-gray-text"
      >Provide some fake details, the data will be refreshed at certain
      interval</span
    >
    ...
    <div>
      <div class="p-mt-4">
        ...
        <Dropdown
          ...
          v-model="recData.category"
          :options="['Camera', 'Laptop', 'Tablet', 'Phone']"
        />
      </div>
      <div class="p-mt-1">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1"
        ...
        <InputText
          type="text"
          v-model="recData.productCode"
          class="p-inputtext-sm p-mr-1"
          style="width: 70px"
        />
        <InputText
          type="text"
          v-model="recData.productName"
          class="p-inputtext-sm"
        />
      </div>
      <div class="p-mt-1">
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      if (resp.data.msgType === "SUCCESS") {
        toast.add({
          severity: "success",
          summary: rawProductObj.id
            ? "Thêm mới thành công!"
            : "Thêm mới thành công!",
          detail: `${rawProductObj.productName} (${rawProductObj.productCode})`,
          life: 3000,
        });
        if (!rawProductObj.id) {
