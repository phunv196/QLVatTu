<template>
  <div>
    ...
    <Sidebar v-model:visible="showSlideOut" position="right" style="width:1100px">
      <DeliveryBillDetails :rec="selectedRec" @cancel="showSlideOut = false" @changed="getData()"
                   :arrWarehouse="arrWarehouse" :arrFactory="arrFactory"
                           :isNew="isNewRec"></DeliveryBillDetails>
    </Sidebar>
    <h3> Quản lý phiếu xuất kho </h3>
    <div class="p-d-flex p-flex-row p-mb-1" style="width:1150px">
    ...
    <DataTable
      ...
      class="p-datatable-sm p-datatable-hoverable-rows m-border p-mb-4" style="width:1150px">
      <Column field="deliveryBillId" header="ID phiếu xuất" headerStyle="width:90px;"></Column>
      <Column field="code" header="Mã phiếu xuất" headerStyle="width:90px"></Column>
      <Column field="name" header="Tên phiếu xuất" headerStyle="width:160px"></Column>
      <Column field="strDateDeliveryBill" header="Ngày lập phiếu" headerStyle="width:160px"></Column>
      <Column field="warehouseCode" header="Mã kho hàng" headerStyle="width:160px;"></Column>
      <Column field="factoryCode" header="Mã phân xưởng" headerStyle="width:160px"></Column>
      <Column field="sumMoney" header="Tổng giao dịch" headerStyle="width:160px"></Column>
...
export default defineComponent({
  setup(): unknown {
    ...
    const getData = async (page: number, requestedPageSize: number, deliveryBillId = '', name = '',
      code = '', formDate = '', toDate = ''
    ) => {
      // isLoading.value = true;
      try {
        debugger
        const resp = await DeliveryBillApi.getDeliveryBills(
          page, requestedPageSize, deliveryBillId, name, code, formDate, toDate);
        ...
        list.value = resp.data.list.map((v:Record<string, unknown>) => {
          ...
          const strDateDeliveryBill = new Intl.DateTimeFormat(['ban', 'id'], { year: 'numeric', month: '2-digit', day: '2-digit' }).format(dt);
          return {
            ...v,
            strDateDeliveryBill,
          };
    ...
    const confirmDialog = (rec: Record<string, unknown>) => {
      ...
      confirm.require({
        ...
        accept: async () => {
          try {
            const resp = await DeliveryBillApi.deleteDeliveryBill(rec.deliveryBillId as string);
    ...
    const onSearchKeyup = debounce(() => getData(
      1, pageSize.value, '', `${searchName.value}`, `${searchCode.value}` ,
      `${searchFormDate.value.toString()}`, `${searchToDate.value.toString()}` ), 400);
    ...
    const onAddClick = async () => {
      ...
      isNewRec.value = true;
      selectedRec.value = { deliveryBillId: sequenceId , dateDeliveryBill: today};
