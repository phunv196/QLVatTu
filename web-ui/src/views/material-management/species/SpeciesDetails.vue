<template>
  <div class="m-font-regular">
    <Toast/>
    <h4>Chủng loại # <span style="color:var(--primary-color)"> {{ recData.speciesId ? recData.speciesId : 'NEW' }} </span></h4>

    <div>
      <div class="p-mt-3">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1">Mã chủng loại</label>
        <InputText type="text" v-model="recData.code" class="p-inputtext-sm p-mr-1" style="width:70px"/>
      </div>
      <div class="p-mt-3">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1">Tên chủng loại </label>
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      if (rawSpeciesObj.speciesId) {
        ...
        resp = await SpeciesApi.addSpecies(rawSpeciesObj);
      }
      if (resp.data.msgType === 'SUCCESS') {
        toast.add({
          severity: 'success',
          summary: rawSpeciesObj.speciesId ? 'Product Updated' : 'Product Added',
          detail: `${rawSpeciesObj.name} (${rawSpeciesObj.code})`,
          life: 3000
        });
        if (!rawSpeciesObj.speciesId) {
