<template>
  <div>
    ...
    <Sidebar v-model:visible="showSlideOut" position="right" style="width:700px">
      <SpeciesDetails :rec="selectedRec" @cancel="showSlideOut = false" @changed="getData()"
...
import SpeciesApi from '@/api/material-management/species-api'; // eslint-disable-line import/no-cycle
import SpeciesDetails from '@/views/material-management/species/SpeciesDetails.vue';
  ...
  setup(): unknown {
    ...
    const confirmDialog = (rec: Record<string, unknown>) => {
      confirm.require({
        ...
        accept: async () => {
          try {
            const resp = await SpeciesApi.deleteSpecies(rec.speciesId as string);
