<template>
  <div class="m-font-regular">
    <Toast/>
    <h4>Phiếu nhập kho # <span style="color:var(--primary-color)"> {{ recData.receiptId ? recData.receiptId : 'NEW' }} </span></h4>

    <div>
      <div class="p-mt-3">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1">Mã phiếu nhập</label>
        <InputText type="text" v-model="recData.code" class="p-inputtext-sm p-mr-1" style="width:30%"/>
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1">Tên phiếu nhập </label>
      ...
      <div class="p-mt-3">
        ...
        <textarea rows="3" v-model="recData.description" class="p-inputtext-sm" maxlength="500" style="width: 70%"/>
      </div>
    </div>
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      if (resp.data.msgType === 'SUCCESS') {
        toast.add({
          severity: 'success',
          summary: rawReceiptObj.receiptId ? 'Product Updated' : 'Product Added',
