<template>
  <div>
    ...
    <Sidebar v-model:visible="showSlideOut" position="right" style="width:700px">
      <PositionDetails :rec="selectedRec" @cancel="showSlideOut = false" @changed="getData()"
                   :isNew="isNewRec"></PositionDetails>
    </Sidebar>
    ...
    <DataTable
      ...
      <Column field="positionId" header="ID chức vụ" headerStyle="width:90px;"></Column>
...
import PositionApi from '@/api/material-management/position-api'; // eslint-disable-line import/no-cycle
import PositionDetails from '@/views/material-management/position/PositionDetails.vue';
...
export default defineComponent({
  setup(): unknown {
    ...
    const getData = async (page: number, requestedPageSize: number, positionId = '' ,code='', name='') => {
      // isLoading.value = true;
      try {
        debugger
        const resp = await PositionApi.getPositions(page, requestedPageSize, positionId, code, name);
    ...
    const confirmDialog = (rec: Record<string, unknown>) => {
      ...
      confirm.require({
        ...
        accept: async () => {
          try {
            const resp = await PositionApi.deletePosition(rec.positionId as string);
