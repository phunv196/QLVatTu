<template>
  <div>
    ...
    <Sidebar v-model:visible="showSlideOut" position="right" style="width:700px">
      <DepartmentDetails :rec="selectedRec" @cancel="showSlideOut = false" @changed="getData()"
                   :isNew="isNewRec"></DepartmentDetails>
    </Sidebar>
    <h3> Quản lý nhà phòng ban </h3>
    ...
    <DataTable
      ...
      <Column field="departmentId" header="ID nhà sản xuất" headerStyle="width:90px;"></Column>
...
import DepartmentApi from '@/api/material-management/department-api'; // eslint-disable-line import/no-cycle
import DepartmentDetails from '@/views/material-management/department/DepartmentDetails.vue';
...
export default defineComponent({
  setup(): unknown {
    ...
    const getData = async (page: number, requestedPageSize: number, departmentId = '',code='', name='', email='', phone='') => {
      // isLoading.value = true;
      try {
        debugger
        const resp = await DepartmentApi.getDepartment(page, requestedPageSize, departmentId,code,name,email,phone);
    ...
    const confirmDialog = (rec: Record<string, unknown>) => {
      ...
      confirm.require({
        ...
        accept: async () => {
          try {
            const resp = await DepartmentApi.deleteDepartment(rec.departmentId as string);
    ...
    const onAddClick = () => {
      isNewRec.value = true;
      selectedRec.value = { departmentId: '' };
