<template>
  <div class="m-font-regular">
    <Toast/>
    <h4>Vật tư nhập # <span style="color:var(--primary-color)"> {{ recData.receiptFlowId ? recData.receiptFlowId : 'NEW' }} </span></h4>
    ...
    <div>
      ...
      <div class="p-mt-3">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1">Nhà sản xuất </label>
        <Dropdown style="width:40%" class="p-inputtext-sm" v-model = "recData.supplierId"
                  :options = arrSupplier
                  :filter = true
                  :showClear= true
                  optionLabel="name"
      ...
      <div class="p-mt-3">
        ...
        <InputNumber type="text" v-model="recData.amount" class="p-inputtext-sm" style="width:40%"/>
      </div>
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      if (resp.data.msgType === 'SUCCESS') {
        toast.add({
          severity: 'success',
          summary: rawReceiptFlowObj.receiptFlowId ? 'Product Updated' : 'Product Added',
