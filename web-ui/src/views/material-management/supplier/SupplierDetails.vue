<template>
  <div class="m-font-regular">
    <Toast/>
    <h4>Supplier # <span style="color:var(--primary-color)"> {{ recData.supplierId ? recData.supplierId : 'NEW' }} </span></h4>
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      if (resp.data.msgType === 'SUCCESS') {
        toast.add({
          severity: 'success',
          summary: rawSupplierObj.supplierId ? 'Product Updated' : 'Product Added',
