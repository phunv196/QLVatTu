<template>
  <div class="m-font-regular">
    <Toast/>
    <h4>Dòng thẻ kho # <span style="color:var(--primary-color)"> {{ recData.warehouseCardFlowId ? recData.warehouseCardFlowId : 'NEW' }} </span></h4>
    <div>
      <div class="p-mt-3" style="text-align: center;" >
        <SelectButton
          ...
          :options = "[{label:'Phiếu nhập', code:1}, {label:'Phiếu xuất', code:0}]"
          optionLabel = "label"
          optionValue = "code"
        />
      </div>
      <div class="p-mt-3" v-if="recData.type == 1 || recData.deliveryBillId && recData.receiptId ==null">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1" style="padding-top: 10px; margin-right: 10px">Phiếu nhập </label>
        <Dropdown style="width:76.3%" class="p-inputtext-sm" v-model = "recData.deliveryBillId"
      ...
      <div class="p-mt-3" v-if="recData.type == 0 || recData.receiptId && recData.deliveryBillId == null">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1" style="padding-top: 10px; margin-right: 10px">Phiếu xuất </label>
        <Dropdown style="width:76.3%" class="p-inputtext-sm" v-model = "recData.receiptId"
                  :options = arrReceipt
                  :filter = true
                  :showClear= true
      ...
      <div class="p-mt-3" >
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1">Số lượng </label>
        <InputNumber type="text" v-model="recData.amount" class="p-inputtext-sm" style="width:76.3%"/>
      </div>
      <div class="p-mt-3">
        <label class="p-d-inline-block m-label-size-2 p-text-right p-mr-1"> Ghi chú </label>
        <textarea rows="3" v-model="recData.description" class="p-inputtext-sm" maxlength="500" style="width: 76.3%"/>
      </div>
    </div>
    <!--button-->
  ...
  setup(props, { emit }): unknown {
    ...
    const onApplyChanges = async () => {
      ...
      if (rawWarehouseCardFlowObj.warehouseCardFlowId) {
        ...
        resp = await WarehouseCardFlowApi.updateWarehouseCardFlow(rawWarehouseCardFlowObj);
      } else {
        if (rawWarehouseCardFlowObj.type) {
          rawWarehouseCardFlowObj.receiptId = null;
        } else {
        ...
        resp = await WarehouseCardFlowApi.addWarehouseCardFlow(rawWarehouseCardFlowObj);
      }
      if (resp.data.msgType === 'SUCCESS') {
        toast.add({
          severity: 'success',
          summary: rawWarehouseCardFlowObj.warehouseCardFlowId ? 'Product Updated' : 'Product Added',
    ...
    const getAmountDelivery = async () => {
      const deliveryBillFlow = await DeliveryBillFlowApi.getDeliveryBillFlow(recData.value.deliveryBillId, suppliesId.value );
      if (deliveryBillFlow.data.length > 0 && suppliesId.value > 0 && recData.value.deliveryBillId) {
    ...
    const getAmountReceipt = async () => {
      const receiptFlow = await ReceiptFlowApi.getReceiptFlow(recData.value.receiptId, suppliesId.value );
      if (receiptFlow.data.length > 0 && suppliesId.value > 0 && recData.value.receiptId) {
