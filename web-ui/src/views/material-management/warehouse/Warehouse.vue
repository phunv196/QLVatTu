<template>
  <div>
    ...
    <Sidebar v-model:visible="showSlideOut" position="right" style="width:700px">
      <WarehouseDetails :rec="selectedRec" @cancel="showSlideOut = false" @changed="getData()"
                   :isNew="isNewRec"></WarehouseDetails>
    </Sidebar>
    ...
    <DataTable
      ...
      <Column field="warehouseId" header="ID kho" headerStyle="width:90px;"></Column>
      ...
      <Column field="phone" header="Số điện thoại" headerStyle="width:160px"></Column>
  ...
  setup(): unknown {
    ...
    const confirmDialog = (rec: Record<string, unknown>) => {
      ...
      confirm.require({
        ...
        accept: async () => {
          try {
            const resp = await WarehouseApi.deleteWarehouse(rec.warehouseId as string);
