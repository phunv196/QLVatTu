...
export default {
  async getEmployees(page = 1, pageSize = 20, employeeId = '', searchCode = "", searchName = "",
    searchEmail = "", searchPhone = "", searchDepartment = "", searchPosition = ""): Promise<AxiosResponse> {
    ...
    if (searchDepartment) { qsParams.searchDepartment = searchDepartment; }
    if (searchPosition) { qsParams.searchPosition = searchPosition; }
    return api.get('/employees', { params: qsParams });
  },

  async export( code = "", fullName = "", email = "", phone = "", departmentId = "", positionId = ""): Promise<AxiosResponse> {
    const qsParams: Record<string, number | string> = {};
    ...
    if (departmentId) { qsParams.departmentId = departmentId; }
    if (positionId) { qsParams.positionId = positionId; }
