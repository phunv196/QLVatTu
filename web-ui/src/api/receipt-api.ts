...
export default {
  async getReceipts(page = 1, pageSize = 20, receiptId = '', searchCode = "", searchName = "", searchEmployee = "",
    searchWarehouse = "", searchFormDate = "", searchToDate = ""): Promise<AxiosResponse> {
    const qsParams: Record<string, number | string> = {};
    if (page) { qsParams.page = page; }
    if (pageSize) { qsParams['page-size'] = pageSize; }
    if (receiptId) { qsParams.id = receiptId; }
  ...
  searchFormDate = "", searchToDate = ""): Promise<AxiosResponse> {
    const qsParams: Record<string, number | string> = {};
