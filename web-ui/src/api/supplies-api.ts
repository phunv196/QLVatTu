...
export default {
  async getSupplies(page = 1, pageSize = 20, suppliesId = '', searchCode = "", searchName = "",
    searchSupplier = "", searchSpecies = "", searchFormPrice = "", searchToPrice = "",
    searchQuality = "", searchUnit = "",): Promise<AxiosResponse> {
    const qsParams: Record<string, number | string> = {};
    if (page) { qsParams.page = page; }
    if (pageSize) { qsParams['page-size'] = pageSize; }
    if (suppliesId) { qsParams.id = suppliesId; }
    if (searchCode) { qsParams.searchCode = searchCode; }
    if (searchName) { qsParams.searchName = searchName; }
    if (searchSupplier) { qsParams.searchSupplier = searchSupplier; }
    if (searchSpecies) { qsParams.searchSpecies = searchSpecies; }
    if (searchFormPrice) { qsParams.searchFormPrice = searchFormPrice; }
    if (searchToPrice) { qsParams.searchToPrice = searchToPrice; }
    if (searchQuality) { qsParams.searchQuality = searchQuality; }
  ...
  async export(searchCode = "", searchName = "", searchSupplier = "", searchSpecies = "", searchFormPrice = "",
    ...
    if (searchSupplier) { qsParams.supplierId = searchSupplier; }
    if (searchSpecies) { qsParams.speciesId = searchSpecies; }
    if (searchFormPrice) { qsParams.formPrice = searchFormPrice; }
    if (searchToPrice) { qsParams.toPrice = searchToPrice; }
    if (searchQuality) { qsParams.qualityId = searchQuality; }
    if (searchUnit) { qsParams.unit = searchUnit; }
