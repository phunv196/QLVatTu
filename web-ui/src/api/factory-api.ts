...
export default {
  async getFactorys(page = 1, pageSize = 20, factoryId = '', searchCode = "", searchName = "", searchEmail = "",
  searchEmployee = "", searchFormDate = "", searchToDate = "", searchFormSuccessDate = "",
  searchToSuccessDate = ""): Promise<AxiosResponse> {
    const qsParams: Record<string, number | string> = {};
    if (page) { qsParams.page = page; }
    if (pageSize) { qsParams['page-size'] = pageSize; }
    if (factoryId) { qsParams.id = factoryId; }
    ...
    if (searchFormSuccessDate) { qsParams.searchFormSuccessDate = searchFormSuccessDate; }
    if (searchToSuccessDate) { qsParams.searchToSuccessDate = searchToSuccessDate; }
    return api.get('/factorys', { params: qsParams });
  },

  async export( searchCode = "", searchName = "", searchEmail = "", searchEmployee = "", searchFormDate = "",
    searchToDate = "", searchFormSuccessDate = "", searchToSuccessDate = ""): Promise<AxiosResponse> {
    const qsParams: Record<string, number | string> = {};
    ...
    if (searchFormSuccessDate) { qsParams.formSuccessDate = searchFormSuccessDate; }
    if (searchToSuccessDate) { qsParams.toSuccessDate = searchToSuccessDate; }
